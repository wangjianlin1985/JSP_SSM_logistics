				    		...
				    		<div class="row">
				    			<div class="col-md-12 top5">
				    				...
				    				<table class="table table-condensed table-hover">
				    					<tr class="success bold"><td>序号</td><td>订单状态id</td><td>订单状态名称</td><td>操作</td></tr>
				    					<% 
				    						/*计算起始序号*/
 										...
 										<tr>
 											<td><%=currentIndex %></td>
 											<td><%=orderState.getOrderStateId() %></td>
 											<td><%=orderState.getOrderStateName() %></td>
 											<td>
 												<a href="<%=basePath  %>OrderState/<%=orderState.getOrderStateId() %>/frontshow"><i class="fa fa-info"></i>&nbsp;查看</a>&nbsp;
 												<a href="#" onclick="orderStateEdit('<%=orderState.getOrderStateId() %>');" style="display:none;"><i class="fa fa-pencil fa-fw"></i>编辑</a>&nbsp;
 												<a href="#" onclick="orderStateDelete('<%=orderState.getOrderStateId() %>');" style="display:none;"><i class="fa fa-trash-o fa-fw"></i>删除</a>
 											</td> 
 										</tr>
 										<%}%>
				    				</table>
    ...
    <div class="modal-content">
      ...
      <div class="modal-body" style="height:450px; overflow: scroll;">
		  ...
		  <div class="form-group">
			 <label for="orderState_orderStateId_edit" class="col-md-3 text-right">订单状态id:</label>
			 <div class="col-md-9"> 
			 	<input type="text" id="orderState_orderStateId_edit" name="orderState.orderStateId" class="form-control" placeholder="请输入订单状态id" readOnly>
			 </div>
		  </div> 
		  <div class="form-group">
		  	 <label for="orderState_orderStateName_edit" class="col-md-3 text-right">订单状态名称:</label>
		  	 <div class="col-md-9">
			    <input type="text" id="orderState_orderStateName_edit" name="orderState.orderStateName" class="form-control" placeholder="请输入订单状态名称">
			 </div>
		  </div>
		</form> 
...
/*弹出修改订单状态界面并初始化数据*/
function orderStateEdit(orderStateId) {
	$.ajax({
		url :  basePath + "OrderState/" + orderStateId + "/update",
		type : "get",
		dataType: "json",
...
/*删除订单状态信息*/
function orderStateDelete(orderStateId) {
	if(confirm("确认删除这个记录")) {
		$.ajax({
			type : "POST",
...
function ajaxOrderStateModify() {
	$.ajax({
		url :  basePath + "OrderState/" + $("#orderState_orderStateId_edit").val() + "/update",
		type : "post",
		dataType: "json",
